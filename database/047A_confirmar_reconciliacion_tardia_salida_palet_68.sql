/*
Paquete 047A - Confirmacion supervisada de la salida del palet 57, cuya fila
NAV 26872 fue observada como Registrado. Base exclusiva: EBIR_MES_TEST.
...
IF OBJECT_ID(N'nav.operaciones',N'U') IS NULL
 ...
 OR OBJECT_ID(N'imp.trabajos_impresion',N'U') IS NULL
    THROW 51180, 'El paquete 047A requiere la cola e impresion MES.', 1;
GO
...
(
    ...
    WHERE n.operacion_nav_id=68 AND n.tipo=N'SALIDA_PALET'
      ...
      AND p.palet_id=57 AND p.numero_palet=5 AND p.cantidad_buena=20
      AND p.es_ultimo=1 AND p.autorizado_por_supervisor_id=48
      AND p.estado=N'CERRADO'
)
    THROW 51181, 'La operacion 68 no cumple el estado exacto observado.', 1;
GO

IF (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=68) <> 24
 OR NOT EXISTS
(
    SELECT 1 FROM nav.intentos_operacion
    WHERE operacion_nav_id=68 AND numero_intento=24
 ...
 OR (SELECT COUNT(*) FROM imp.etiquetas
     WHERE etiqueta_id=59 AND orden_id=41 AND palet_id=57
       AND tipo=N'PALET' AND estado=N'PENDIENTE_NAV') <> 1
 OR EXISTS
(
    SELECT 1 FROM imp.etiquetas
    WHERE palet_id=57 AND etiqueta_id<>59
)
 OR EXISTS
(
    SELECT 1 FROM imp.trabajos_impresion WHERE etiqueta_id=59
)
    THROW 51182, 'Intentos, etiqueta o impresion de la operacion 68 cambiaron.', 1;
GO
...
(
    ...
    WHERE p.palet_id=57 AND p.orden_id=41
      AND li.asignado_hasta_utc IS NULL AND li.es_principal=1 AND i.activa=1
) <> 1
    THROW 51183, 'La linea no tiene exactamente una impresora principal activa.', 1;
GO
...
BEGIN TRY
    ...
    (
        ...
        WHERE etiqueta_id=59
    )
        THROW 51184, 'El estado protegido cambio durante 047A.', 1;

    EXEC nav.confirmar_salida_palet
        @operacion_nav_id=68,
        @respuesta=N'{"adapter":"NavisionSoapPalletOutputSender","outcome":"Confirmed","reconciliationMode":"SUPERVISED_EXISTING_OUTPUT","observedNavState":"Registrado","externalIdentifier":"26872"}',
        @identificador_externo=N'26872',
        @correlacion_id=@correlacion_id;

    IF NOT EXISTS
    (
        SELECT 1 FROM nav.operaciones
        WHERE operacion_nav_id=68 AND orden_id=41 AND palet_id=57
          AND tipo=N'SALIDA_PALET' AND estado=N'CONFIRMADA'
          AND numero_intentos=25 AND identificador_externo=N'26872'
     ...
     OR (SELECT COUNT(*) FROM imp.trabajos_impresion
         WHERE etiqueta_id=59 AND es_reimpresion=0 AND estado=N'PENDIENTE') <> 1
        THROW 51185, 'La validacion final de 047A no es correcta.', 1;
