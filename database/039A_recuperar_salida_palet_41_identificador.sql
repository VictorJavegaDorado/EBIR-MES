/*
Paquete 039A - Recuperacion del identificador externo de la operacion 41.
Base exclusiva: EBIR_MES_TEST.

No contacta NAV ni reenvia la salida. Vincula exclusivamente la operacion 41
con la fila OData 26846 ya observada despues del intento 1, para que el Worker
...
IF OBJECT_ID(N'nav.operaciones', N'U') IS NULL
 ...
 OR OBJECT_ID(N'nav.reservar_siguiente_salida_palet', N'P') IS NULL
    THROW 51117, 'El paquete 039A requiere 038A, impresion y la auditoria MES.', 1;
GO
...
(
    ...
    WHERE n.operacion_nav_id=41 AND n.tipo=N'SALIDA_PALET'
      ...
      AND o.estado=N'PENDIENTE_CIERRE'
      AND o.cantidad_objetivo=100 AND o.cantidad_buena_acumulada=100
      AND o.cantidad_reservada_activa=0
      AND p.palet_id=31 AND p.numero_palet=5 AND p.cantidad_buena=20
      AND p.es_ultimo=1 AND p.autorizado_por_supervisor_id=48
      AND p.estado=N'CERRADO'
)
    THROW 51118, 'La operacion 41 no cumple el estado exacto posterior al intento 1.', 1;
GO

IF (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=41) <> 1
 OR NOT EXISTS
(
    SELECT 1 FROM nav.intentos_operacion
    WHERE operacion_nav_id=41 AND numero_intento=1
      ...
      AND TRY_CONVERT(int,JSON_VALUE(respuesta,'$.baselineMaximumId'))=26845
)
    THROW 51119, 'El intento de la operacion 41 no coincide con la salida observada.', 1;
GO

IF (SELECT COUNT(*) FROM imp.etiquetas
    WHERE etiqueta_id=33 AND orden_id=31 AND palet_id=31
      AND tipo=N'PALET' AND estado=N'PENDIENTE_NAV') <> 1
 OR EXISTS
(
    SELECT 1 FROM imp.etiquetas
    WHERE palet_id=31 AND etiqueta_id<>33
)
 OR EXISTS
(
    SELECT 1 FROM imp.trabajos_impresion WHERE etiqueta_id=33
)
    THROW 51120, 'La etiqueta 33 o sus trabajos no cumplen el estado exacto previo.', 1;
GO
...
(
    ...
    WHERE tipo_evento=N'NAV_SALIDA_IDENTIFICADOR_RECUPERADO'
      AND entidad=N'NAV_OPERACION' AND entidad_id=41
)
    THROW 51121, 'El identificador de la operacion 41 ya fue recuperado.', 1;
GO
...
BEGIN TRY
    ...
    (
        ...
        WHERE operacion_nav_id=41 AND tipo=N'SALIDA_PALET'
    ...
    IF @orden_id IS NULL
        THROW 51122, 'La operacion cambio durante la recuperacion 039A.', 1;
    ...
    (
        ...
        WHERE p.palet_id=31 AND p.orden_id=@orden_id AND p.numero_palet=5
          AND p.cantidad_buena=20 AND p.es_ultimo=1
          AND p.autorizado_por_supervisor_id=48 AND p.estado=N'CERRADO'
          AND o.orden_id=31 AND o.numero_orden=N'FL26-00004'
          AND o.producto_codigo=N'27920LG'
          AND o.estado=N'PENDIENTE_CIERRE'
          AND o.cantidad_objetivo=100 AND o.cantidad_buena_acumulada=100
    ...
    (
        ...
        WHERE etiqueta_id=33
    )
        THROW 51123, 'El palet, orden, intento, etiqueta o cola cambiaron durante 039A.', 1;
    ...
    WHERE operacion_nav_id=41 AND estado=N'RESULTADO_DESCONOCIDO'
      AND numero_intentos=1 AND identificador_externo IS NULL
      AND reservado_utc IS NULL AND reservado_por IS NULL;
    IF @@ROWCOUNT <> 1
        THROW 51124, 'No se recupero exactamente una operacion.', 1;
    ...
    (N'NAV_SALIDA_IDENTIFICADOR_RECUPERADO',SUSER_SNAME(),@orden_id,
     ...
     N'{"estado":"RESULTADO_DESCONOCIDO","numero_intentos":1,"identificador_externo":"26846","modo":"RECONCILIACION"}',
    ...
    (
        ...
        WHERE operacion_nav_id=41 AND orden_id=31 AND palet_id=31
          AND estado=N'RESULTADO_DESCONOCIDO' AND numero_intentos=1
          AND identificador_externo=N'26846'
          AND proximo_intento_utc IS NOT NULL AND procesada_utc IS NULL
          AND reservado_utc IS NULL AND reservado_por IS NULL
    )
     OR (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=41) <> 1
     OR (SELECT COUNT(*) FROM imp.etiquetas
         WHERE etiqueta_id=33 AND orden_id=31 AND palet_id=31
           AND tipo=N'PALET' AND estado=N'PENDIENTE_NAV') <> 1
     OR EXISTS (SELECT 1 FROM imp.trabajos_impresion WHERE etiqueta_id=33)
     OR (SELECT COUNT(*) FROM aud.eventos
         WHERE tipo_evento=N'NAV_SALIDA_IDENTIFICADOR_RECUPERADO'
           AND entidad=N'NAV_OPERACION' AND entidad_id=41
           AND orden_id=@orden_id) <> 1
        THROW 51125, 'La validacion final de 039A no es correcta.', 1;
