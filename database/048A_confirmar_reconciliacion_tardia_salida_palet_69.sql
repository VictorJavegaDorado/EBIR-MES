/*
Paquete 048A - Confirmacion supervisada de la salida del palet 58, cuya fila
NAV 26873 fue observada como Registrado. Base exclusiva: EBIR_MES_TEST.
...
IF OBJECT_ID(N'nav.operaciones',N'U') IS NULL
 ...
 OR OBJECT_ID(N'imp.trabajos_impresion',N'U') IS NULL
    THROW 51190, 'El paquete 048A requiere la cola e impresion MES.', 1;
GO
...
(
    ...
    WHERE n.operacion_nav_id=69 AND n.tipo=N'SALIDA_PALET'
      AND n.estado=N'RESULTADO_DESCONOCIDO' AND n.numero_intentos=24
      AND n.identificador_externo=N'26873'
      AND n.proximo_intento_utc IS NULL AND n.procesada_utc IS NOT NULL
      AND n.reservado_utc IS NULL AND n.reservado_por IS NULL
      AND o.orden_id=42 AND o.numero_orden=N'FL26-00015'
      AND o.producto_codigo=N'27920LG' AND o.estado=N'ABIERTA'
      AND o.cantidad_objetivo=100 AND o.cantidad_buena_acumulada=20
      AND o.cantidad_reservada_activa=20
      AND p.palet_id=58 AND p.numero_palet=1 AND p.cantidad_buena=20
      AND p.es_ultimo=0 AND p.autorizado_por_supervisor_id IS NULL
      AND p.estado=N'CERRADO'
)
    THROW 51191, 'La operacion 69 no cumple el estado exacto observado.', 1;
GO

IF (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=69) <> 24
 OR NOT EXISTS
(
    SELECT 1 FROM nav.intentos_operacion
    WHERE operacion_nav_id=69 AND numero_intento=24
 ...
 OR (SELECT COUNT(*) FROM imp.etiquetas
     WHERE etiqueta_id=60 AND orden_id=42 AND palet_id=58
       AND tipo=N'PALET' AND estado=N'PENDIENTE_NAV') <> 1
 OR EXISTS
(
    SELECT 1 FROM imp.etiquetas
    WHERE palet_id=58 AND etiqueta_id<>60
)
 OR EXISTS
(
    SELECT 1 FROM imp.trabajos_impresion WHERE etiqueta_id=60
)
    THROW 51192, 'Intentos, etiqueta o impresion de la operacion 69 cambiaron.', 1;
GO
...
(
    ...
    WHERE p.palet_id=58 AND p.orden_id=42
      AND li.asignado_hasta_utc IS NULL AND li.es_principal=1 AND i.activa=1
) <> 1
    THROW 51193, 'La linea no tiene exactamente una impresora principal activa.', 1;
GO
...
BEGIN TRY
    ...
    (
        ...
        WHERE etiqueta_id=60
    )
        THROW 51194, 'El estado protegido cambio durante 048A.', 1;

    EXEC nav.confirmar_salida_palet
        @operacion_nav_id=69,
        @respuesta=N'{"adapter":"NavisionSoapPalletOutputSender","outcome":"Confirmed","reconciliationMode":"SUPERVISED_EXISTING_OUTPUT","observedNavState":"Registrado","externalIdentifier":"26873"}',
        @identificador_externo=N'26873',
        @correlacion_id=@correlacion_id;

    IF NOT EXISTS
    (
        SELECT 1 FROM nav.operaciones
        WHERE operacion_nav_id=69 AND orden_id=42 AND palet_id=58
          AND tipo=N'SALIDA_PALET' AND estado=N'CONFIRMADA'
          AND numero_intentos=25 AND identificador_externo=N'26873'
     ...
     OR (SELECT COUNT(*) FROM imp.trabajos_impresion
         WHERE etiqueta_id=60 AND es_reimpresion=0 AND estado=N'PENDIENTE') <> 1
        THROW 51195, 'La validacion final de 048A no es correcta.', 1;
