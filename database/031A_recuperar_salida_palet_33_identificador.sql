/*
Paquete 031A - Recuperacion del identificador externo de la operacion 33.
Base exclusiva: EBIR_MES_TEST.

No contacta NAV ni reenvia la salida. Vincula exclusivamente la operacion 33
con la fila OData 26839 observada despues del intento 1, para que el Worker
...
IF OBJECT_ID(N'nav.operaciones', N'U') IS NULL
 ...
 OR OBJECT_ID(N'nav.reservar_siguiente_salida_palet', N'P') IS NULL
    THROW 51072, 'El paquete 031A requiere 028A y la auditoria MES.', 1;
GO
...
(
    ...
    WHERE n.operacion_nav_id=33 AND n.tipo=N'SALIDA_PALET'
      ...
      AND p.palet_id=23 AND p.numero_palet=3 AND p.cantidad_buena=20
      AND p.es_ultimo=0 AND p.estado=N'CERRADO'
)
    THROW 51073, 'La operacion 33 no cumple el estado exacto posterior al intento 1.', 1;
GO

IF (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=33) <> 1
 OR NOT EXISTS
(
    SELECT 1 FROM nav.intentos_operacion
    WHERE operacion_nav_id=33 AND numero_intento=1
      ...
      AND TRY_CONVERT(int,JSON_VALUE(respuesta,'$.baselineMaximumId'))=26838
)
    THROW 51074, 'El intento 1 no coincide con la salida no observada esperada.', 1;
GO
...
(
    ...
    WHERE tipo_evento=N'NAV_SALIDA_IDENTIFICADOR_RECUPERADO'
      AND entidad=N'NAV_OPERACION' AND entidad_id=33
)
    THROW 51075, 'El identificador de la operacion 33 ya fue recuperado.', 1;
GO
...
BEGIN TRY
    ...
    (
        ...
        WHERE operacion_nav_id=33 AND tipo=N'SALIDA_PALET'
    ...
    IF @orden_id IS NULL
        THROW 51076, 'La operacion cambio durante la recuperacion 031A.', 1;

    UPDATE nav.operaciones
    SET identificador_externo=N'26839',
        proximo_intento_utc=SYSUTCDATETIME(),
        procesada_utc=NULL,reservado_utc=NULL,reservado_por=NULL
    WHERE operacion_nav_id=33 AND estado=N'RESULTADO_DESCONOCIDO'
      AND numero_intentos=1 AND identificador_externo IS NULL
      AND reservado_utc IS NULL AND reservado_por IS NULL;
    IF @@ROWCOUNT <> 1
        THROW 51077, 'No se recupero exactamente una operacion.', 1;
    ...
    (N'NAV_SALIDA_IDENTIFICADOR_RECUPERADO',SUSER_SNAME(),@orden_id,
     ...
     N'{"estado":"RESULTADO_DESCONOCIDO","numero_intentos":1,"identificador_externo":"26839","modo":"RECONCILIACION"}',
    ...
    (
        ...
        WHERE operacion_nav_id=33 AND estado=N'RESULTADO_DESCONOCIDO'
          AND numero_intentos=1 AND identificador_externo=N'26839'
          AND proximo_intento_utc IS NOT NULL AND procesada_utc IS NULL
          AND reservado_utc IS NULL AND reservado_por IS NULL
    )
     OR (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=33) <> 1
     OR (SELECT COUNT(*) FROM aud.eventos
         WHERE tipo_evento=N'NAV_SALIDA_IDENTIFICADOR_RECUPERADO'
           AND entidad=N'NAV_OPERACION' AND entidad_id=33
           AND orden_id=@orden_id) <> 1
        THROW 51078, 'La validacion final de 031A no es correcta.', 1;
