/*
Paquete 044A - Confirmacion supervisada de la salida de palet 50, cuya fila
NAV 26854 fue observada como Registrado. Base exclusiva: EBIR_MES_TEST.
...
IF OBJECT_ID(N'nav.operaciones',N'U') IS NULL
 ...
 OR OBJECT_ID(N'imp.trabajos_impresion',N'U') IS NULL
    THROW 51160, 'El paquete 044A requiere la cola e impresion MES.', 1;
GO
...
(
    ...
    WHERE n.operacion_nav_id=50 AND n.tipo=N'SALIDA_PALET'
      ...
      AND p.palet_id=39 AND p.numero_palet=1 AND p.cantidad_buena=20
      AND p.es_ultimo=0 AND p.autorizado_por_supervisor_id IS NULL
      AND p.estado=N'CERRADO'
)
    THROW 51161, 'La operacion 50 no cumple el estado exacto observado.', 1;
GO

IF (SELECT COUNT(*) FROM nav.intentos_operacion WHERE operacion_nav_id=50) <> 12
 OR NOT EXISTS
(
    SELECT 1 FROM nav.intentos_operacion
    WHERE operacion_nav_id=50 AND numero_intento=12
      AND resultado=N'RESULTADO_DESCONOCIDO'
      AND error_normalizado=N'RESULTADO_DESCONOCIDO'
      AND JSON_VALUE(respuesta,'$.reason')=N'OutputStillPending'
)
 OR (SELECT COUNT(*) FROM imp.etiquetas
     WHERE etiqueta_id=41 AND orden_id=36 AND palet_id=39
       AND tipo=N'PALET' AND estado=N'PENDIENTE_NAV') <> 1
 OR EXISTS
(
    SELECT 1 FROM imp.etiquetas
    WHERE palet_id=39 AND etiqueta_id<>41
)
 OR EXISTS
(
    SELECT 1 FROM imp.trabajos_impresion WHERE etiqueta_id=41
)
    THROW 51162, 'Intentos, etiqueta o impresion de la operacion 50 cambiaron.', 1;
GO
...
(
    ...
    WHERE p.palet_id=39 AND p.orden_id=36
      AND li.asignado_hasta_utc IS NULL AND li.es_principal=1 AND i.activa=1
) <> 1
    THROW 51163, 'La linea no tiene exactamente una impresora principal activa.', 1;
GO
...
BEGIN TRY
    ...
    (
        ...
        WHERE etiqueta_id=41
    )
        THROW 51164, 'El estado protegido cambio durante 044A.', 1;

    EXEC nav.confirmar_salida_palet
        @operacion_nav_id=50,
        @respuesta=N'{"adapter":"NavisionSoapPalletOutputSender","outcome":"Confirmed","reconciliationMode":"SUPERVISED_EXISTING_OUTPUT","observedNavState":"Registrado","externalIdentifier":"26854"}',
        @identificador_externo=N'26854',
        @correlacion_id=@correlacion_id;

    IF NOT EXISTS
    (
        SELECT 1 FROM nav.operaciones
        WHERE operacion_nav_id=50 AND orden_id=36 AND palet_id=39
          AND tipo=N'SALIDA_PALET' AND estado=N'CONFIRMADA'
          AND numero_intentos=13 AND identificador_externo=N'26854'
     ...
     OR (SELECT COUNT(*) FROM imp.trabajos_impresion
         WHERE etiqueta_id=41 AND es_reimpresion=0 AND estado=N'PENDIENTE') <> 1
        THROW 51165, 'La validacion final de 044A no es correcta.', 1;
