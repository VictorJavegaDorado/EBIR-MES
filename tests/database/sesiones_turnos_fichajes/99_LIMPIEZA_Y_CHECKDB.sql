/*
Pruebas 011 - limpieza total y comprobacion de integridad.
...
IF DB_NAME() <> N'EBIR_MES_TEST'
    THROW 53900, 'Pruebas permitidas unicamente en EBIR_MES_TEST.', 1;
...
FROM nav.empresas
WHERE codigo = N'ZZTEST_011';

IF (SELECT COUNT(*) FROM @ordenes) <> 4
 OR (SELECT COUNT(*) FROM @lineas) <> 6
 OR (SELECT COUNT(*) FROM @empleados) <> 5
 OR (SELECT COUNT(*) FROM @impresoras) <> 1
 OR (SELECT COUNT(*) FROM @empresas) <> 1
    THROW 53901, 'Los fixtures 011 no existen o estan incompletos; no se limpia.', 1;
...
BEGIN TRY
    ...
    /* Logistica, aunque 011 no crea registros en estas tablas. */
    DELETE hs
    ...
    IF EXISTS (SELECT 1 FROM prod.ordenes WHERE numero_orden LIKE N'ZZ11-%')
     OR EXISTS (SELECT 1 FROM cfg.lineas WHERE codigo LIKE N'ZZ11-%')
     ...
     (
        ...
        THROW 53902, 'La limpieza de identificadores 011 fue incompleta.', 1;
    ...
    IF (SELECT COUNT(*) FROM cfg.centros_trabajo) <> 1
     ...
     OR (SELECT COUNT(*) FROM [log].motivos_scrap) <> 30
        THROW 53903, 'Los catalogos iniciales no conservan sus 37 registros.', 1;
    ...
    */
    ...
    IF @filas_operativas <> 0
        THROW 53904, 'Quedan filas fuera de los cinco catalogos iniciales.', 1;

    IF (SELECT COUNT(*) FROM sys.tables WHERE is_ms_shipped = 0) <> 37
        THROW 53905, 'El numero de tablas de usuario no es 37.', 1;

    IF (SELECT COUNT(*) FROM sys.procedures WHERE is_ms_shipped = 0) <> 11
        THROW 53906, 'El numero de procedimientos de usuario no es 11.', 1;
    ...
    (
        ...
        THROW 53907, 'La limpieza altero la pertenencia de EBIR\MES$ a mes_runtime.', 1;
...
PRINT N'PRUEBAS 011 LIMPIEZA: OK';

DBCC CHECKDB (N'EBIR_MES_TEST')
WITH NO_INFOMSGS, ALL_ERRORMSGS;

PRINT N'PRUEBAS 011 DBCC CHECKDB: OK';
