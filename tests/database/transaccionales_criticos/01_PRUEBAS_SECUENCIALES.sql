...
IF DB_NAME()<>N'EBIR_MES_TEST' THROW 52100,'Base no autorizada.',1;
...
IF @op1 IS NULL OR @op2 IS NULL OR @sup IS NULL THROW 52101,'Fixtures ausentes.',1;
...
UPDATE prod.estados_linea SET sesion_linea_id=@s1,estado=N'PRODUCIENDO',motivo_bloqueo=NULL WHERE linea_id=@l1;
DECLARE @r1 bigint;
EXEC prod.reservar_palet @o1,@s1,20,@op1,@corr,@r1 OUTPUT;
IF NOT EXISTS(SELECT 1 FROM prod.reservas_palet WHERE reserva_palet_id=@r1 AND estado=N'ACTIVA')
 THROW 52102,'Caso 1 fallido.',1;
PRINT N'OK 1 - Reserva valida';

/* 2 Sobre-reserva secuencial. */
BEGIN TRY
 DECLARE @rx bigint; EXEC prod.reservar_palet @o1,@s1,20,@op1,@corr,@rx OUTPUT;
 THROW 52103,'Caso 2: segunda reserva admitida.',1;
END TRY
BEGIN CATCH
 IF ERROR_NUMBER()<>51204 THROW;
END CATCH;
PRINT N'OK 2a - Sobre-reserva secuencial rechazada';
